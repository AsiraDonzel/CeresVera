...
export default function Dashboard() {
    ...
    const recentScans = [
        { id: 1, date: 'Oct 12, 2026', crop: 'Tomato', status: 'Healthy', icon: <CheckCircle className="w-5 h-5 text-green-500" /> },
        { id: 2, date: 'Oct 10, 2026', crop: 'Maize', status: 'Leaf Blight', icon: <AlertTriangle className="w-5 h-5 text-amber-500" /> },
        { id: 3, date: 'Oct 05, 2026', crop: 'Cassava', status: 'Healthy', icon: <CheckCircle className="w-5 h-5 text-green-500" /> },
    ];
                ...
                <div className="lg:col-span-2 space-y-8">
                            ...
                            {[
                                ...
                                <div key={i} className="flex-1 h-full flex flex-col justify-end group relative z-10 cursor-pointer pb-7">
                                    ...
                                    >
                                        ...
                                        <span className={`${item.val >= 80 ? 'text-sage-400' :
                                                item.val >= 50 ? 'text-amber-400' : 'text-red-400'
                                            }`}>{item.label}</span>
                                    ...
                                    <div className="w-full max-w-[48px] mx-auto bg-gray-50/50 rounded-xl relative h-[calc(100%-1.75rem)] flex items-end overflow-hidden group-hover:bg-gray-100 transition-colors">
                                        ...
                                        <motion.div
                                            ...
                                            className={`w-full rounded-xl transition-all duration-300 relative shadow-inner ${item.val >= 80 ? 'bg-gradient-to-t from-sage-600 to-sage-400 group-hover:from-sage-700 group-hover:to-sage-500' :
                                                    item.val >= 50 ? 'bg-gradient-to-t from-amber-500 to-amber-300 group-hover:from-amber-600 group-hover:to-amber-400' :
                                                        'bg-gradient-to-t from-rose-500 to-rose-400 group-hover:from-rose-600 group-hover:to-rose-500'
                                                }`}
                                        >
                    ...
                    <motion.div variants={itemVariants} className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                        <div className="p-8 border-b border-gray-50 flex justify-between items-center">
                            ...
                            <button className="text-sage-700 font-bold text-sm hover:text-sage-900 transition-colors flex items-center gap-1">View All <ChevronRight className="w-4 h-4" /></button>
                        </div>
                        <div className="divide-y divide-gray-50">
                            {recentScans.map((scan) => (
                                <div key={scan.id} className="p-6 hover:bg-gray-50 transition-colors flex items-center justify-between group cursor-pointer">
                                    <div className="flex items-center gap-5">
                                        ...
                                        <div>
                                            <div className="font-bold text-gray-900 text-lg">{scan.crop}</div>
                                            <div className="text-sm text-gray-500 font-medium mt-1 flex items-center gap-1.5">
                                                <Clock className="w-3.5 h-3.5" /> {scan.date}
                                            </div>
                                        </div>
                                    </div>
                                    <div className="text-right flex items-center gap-4">
                                        <span className={`px-4 py-1.5 rounded-full text-xs font-bold tracking-wide uppercase ${scan.status === 'Healthy' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700 border border-amber-200'}`}>
                                            {scan.status}
                                        </span>
                                    </div>
                                </div>
